...
public class BrickBreakerGame implements IBaseGame {
	...
	private void _load(Context context) {
		...
		paddle.setPosition(GameConfig.SCREEN_WIDTH * 0.5f,
				GameConfig.SCREEN_HEIGHT - paddle.getHeight() * 0.5f);
		ball.reset();
	...
	private void _colisiones() {
		...
		for (Block block : blocks) {
			if (block.getState() != Block.STATE_DESTROYED
					&& ball.getBounds().intersect(block.getBounds())) {
				score++;
	...
	public void draw(Canvas canvas) {
		...
		canvas.drawText("Puntaje: " + score, GameConfig.SCREEN_WIDTH * 0.5f,
				300, p_score);
